...
import { getDoc, doc, collection, updateDoc, query, getDocs, where, setDoc } from 'firebase/firestore';
  ...
  return (
    <>
      ...
      <div className="p-3 m-auto space-y-3 border-2">
        ...
        <p> 베팅할 코인개수: <input type="number" className="border-2" value={betAmount} onChange={(e) => setBetAmount(Math.max(10, parseInt(e.target.value)) || 0)} step="10" min="10" defaultValue="10" /> </p>

          <div className="flex justify-center space-x-3">
            <button className="p-10 text-xl text-white bg-green-600 shadow-md rounded-xl shadow-black" onClick={() => handleBet(true)}>홀</button>
            <button className="p-10 text-xl text-white bg-green-600 shadow-md rounded-xl shadow-black" onClick={() => handleBet(false)}>짝</button>
          </div>
        </div>
      </div>
      </div>
